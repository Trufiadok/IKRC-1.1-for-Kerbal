    ...
    public class IkRobotController : PartModule
    {
        ...
        private void StatesToThetas()
        {
            ...
            if (SortIkServo != null)
                ...
                Debug.Log(string.Format("[IKRC] StatesToThetas() SortIkServo == null"));
        }
        ...
        private bool GetArmServos()
        {
            ...
            if (IRWrapper.APIReady)
            {
                ...
                if (SortIkServo == null)
                {
                    ...
                    Debug.Log(string.Format("[IKRC] - GetArmServos() - construct SortIkServo"));
                }
                Debug.Log(string.Format("[IKRC] {0} - Inited ServoLists", 21));
                ...
                if (AllIkServo.Count >= (JointList.Length - 1))
                {
                    ...
                    Debug.Log(string.Format("[IKRC] {0} - END - SortIkServo.Add(new IKservo(lastPart)", 41));
                    Debug.Log(string.Format("[IKRC] {0} - END - SortIkServo.Count = {1}", 41, SortIkServo.Count));
                    ...
                    if (minMaxAnglesList.Count == SortIkServo.Count)
                        ...
                        Debug.Log(string.Format("[IKRC] - GetArmServos() - minMaxAnglesList.Count != SortIkServo.Count"));

                    foreach (IKservo element in SortIkServo)
                    {
                        Debug.Log(string.Format("[IKRC] - GetArmServos() - SortIkServo.MinAngle = {0} ~.MaxAngle = {1}", element.MinAngle, element.MaxAngle));
                    }
                }
                else
                {
        ...
        private void RemoveHingeServo()
        {
            ...
            Debug.Log(string.Format("[IKRC] {0} - END RemoveHingeServo()", 23));
        }
        ...
        //        {
        //            AllIkServo.Add(new IKservo(part, iservo));
        //            Debug.Log(string.Format("[IKRC] {0} - AllIkServo.Add( " + part.name + " )", 23));

        //        }
        //    }

        //    Debug.Log(string.Format("[IKRC] {0} - END RemoveFalseServo()", 23));
        //}
        ...
        public void Start()
        {
            ...
            if (HighLogic.LoadedSceneIsFlight)
            {
                ...
                if (!DumpServoStructure(SortIkServo))
                    return;
                Debug.Log(string.Format("[IKRC] >>> >>> Start() - DumpServoStructure(SortIkServo) == true"));
                if (!IsLoadedFkParams)
                {
                ...
                //foreach (IKservo element in SortIkServo)
                //{
                //    Debug.Log(string.Format("[IKRC] - Start() - SortIkServo[{0}].MinAngle = {1} ~.MaxAngle = {2}", j, SortIkServo[j].MinAngle, SortIkServo[j].MaxAngle));
                //    j++;
                //}
        ...
        public void Update()
        {
            ...
            if (HighLogic.LoadedSceneIsFlight && IsInitedModule && IsInitedModule1st)
            {
                ...
                //foreach (IKservo element in SortIkServo)
                //{
                //    Debug.Log(string.Format("[IKRC] - Update() IsInitedModule1st - SortIkServo[{0}].MinAngle = {1} ~.MaxAngle = {2}", k, SortIkServo[k].MinAngle, SortIkServo[k].MaxAngle));
                //    k++;
                //}
            }
            ...
            if (ircWindowActive && HighLogic.LoadedSceneIsFlight && IsInitedModule)
            {
                ...
                if (veeStateBool && (dockingNodeObject == null) && VeeIsActive)
                {
                    ...
                    Debug.Log(string.Format("[IKRC] {0} - Update() - (SortIkServo[SortIkServo.Count - 1]) " + IKServoToString(SortIkServo[SortIkServo.Count - 1]), 601));
        ...
        public void DumpServoStructureEvent()
        {
            ...
            if (IRWrapper.APIReady)
            {
                Debug.Log(string.Format("[IKRC] {0} - DumpServoStructureEvent() - IRWrapper.APIReady", 21));
                AllIkServo = new List<IKservo>();
                allServos = new List<IRWrapper.IServo>();
                SortIkServo = new List<IKservo>();
                Debug.Log(string.Format("[IKRC] {0} - DumpServoStructureEvent() - Inited ServoLists", 21));
        ...
        public override void OnSave(ConfigNode node)
        {
            ...
            else
            {
                ...
                Debug.Log(string.Format("[IKRC] - OnLoad() IsInitedModule = {0}", IsInitedModule.ToString()));
            }
        ...
        private bool GetFKParamsFromBuffer()
        {
            if (IsLoadedFkParams)
            {
                ...
                foreach (IKservo ikServo in FKparamsIkServo)
                {
                    ...
                    Debug.Log(string.Format("[IKRC] {0} - ParentOffset " + VectorToString(SortIkServo[servoNumber].fkParams.ParentOffset, "0.00"), 42));
                    Debug.Log(string.Format("[IKRC] {0} - Axis " + VectorToString(ikServo.fkParams.Axis, "0.0"), 40));
                    Debug.Log(string.Format("[IKRC] {0} - Rotation " + QuaternionToString(SortIkServo[servoNumber].fkParams.Rotation, "0.00"), 42));
        ...
        private bool ImplementServoRotation(List<IKservo> Servos, float[] theta, float speed = 0.25f)
        {
            ...
            for (int j = 0; j < SortIkServo.Count - 1; j++)
                if (!double.IsNaN(theta[j]))
                {
                    ...
                    if (servoTheta != SortIkServo[j].iservo.Position)
                    {
        ...
        private bool ImplementNormalServoRotation(List<IKservo> Servos, float[] currentTheta, float[] theta, float maxspeed)
        {
            ...
            for (int j = 0; j < SortIkServo.Count - 1; j++)
            {
                ...
                // eliminating bouncing noise - if servo target position the same that current position then don't send move command. 
                if (servoTheta[j] != SortIkServo[j].iservo.Position)
                {
                    //SortIkServo[j].iservo.SetModulo(SLTBreaking);
